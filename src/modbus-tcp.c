...
static int _modbus_tcp_init_win32(void)
{
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        ESP_LOGE(TAG, "WSAStartup() returned error code %d\n",
...
#endif
*/
static const char *TAG = "libmodbus-modbus-tcp";
                                              ...
                                              const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[0] != rsp[0] || req[1] != rsp[1]) {
        if (ctx->debug) {
            ESP_LOGE(TAG, "Invalid transaction ID received 0x%X (not 0x%X)\n",
    ...
    if (rsp[2] != 0x0 && rsp[3] != 0x0) {
        if (ctx->debug) {
            ESP_LOGE(TAG, "Invalid protocol ID received 0x%X (not 0x0)\n",
                    (rsp[2] << 8) + rsp[3]);
        }
        errno = EMBBADDATA;
        return -1;
...
int modbus_tcp_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        ESP_LOGI(TAG,"The client connection from %s is accepted\n",
...
int modbus_tcp_pi_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        ESP_LOGI(TAG,"The client connection is accepted.\n");
    }
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    if (sigaction(SIGPIPE, &sa, NULL) < 0) {
        ...
        ESP_LOGE(TAG, "Could not install SIGPIPE handler.\n");
        return NULL;
    }
#endif
    ...
    if (ip != NULL) {
        ...
        if (ret_size == 0) {
            ESP_LOGE(TAG, "The IP string is empty\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
        ...
        if (ret_size >= dest_size) {
            ESP_LOGE(TAG, "The IP string has been truncated\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    if (node == NULL) {
        ...
        if (ret_size == 0) {
            ESP_LOGE(TAG, "The node string is empty\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
        ...
        if (ret_size >= dest_size) {
            ESP_LOGE(TAG, "The node string has been truncated\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
    ...
    if (ret_size == 0) {
        ESP_LOGE(TAG, "The service string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        ESP_LOGE(TAG, "The service string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
