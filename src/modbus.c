...
#define LIBMODBUS_VERSION_STRING "TestVerison"
/* Exported version */
...
static const char *TAG = "libmodbus-modbus";
...
static void _sleep_response_timeout(modbus_t *ctx)
{
    ...
    usleep((ctx->response_timeout.tv_sec*1000*1000)+ctx->response_timeout.tv_usec);
#endif
}
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
	...
	printf("Length to read is %d\n",length_to_read);

    if (msg_type == MSG_INDICATION) {
        /* Wait for a message, we don't know when the message will be
         * received */
		 printf("msg type: Message Indication\n");
		...
		printf("msg type: No Message Indication\n");
        tv.tv_sec = ctx->response_timeout.tv_sec;
        tv.tv_usec = ctx->response_timeout.tv_usec;
        p_tv = &tv;
    }
	ESP_LOGI(TAG,"getting into the loop\n");
    while (length_to_read != 0) {
        printf("Socket select: Waiting for incoming message");
		...
		printf("At tcp receive:\n");

        rc = ctx->backend->recv(ctx, msg + msg_length, length_to_read);
		printf("Value of rc at tcp recv is %d\n",rc);
        if (rc == 0) {
            errno = ECONNRESET;
            rc = -1;
        }
        ...
        length_to_read -= rc;
		printf("Remaining bytes is %d\n",length_to_read);

        if (length_to_read == 0) {
			ESP_LOGI(TAG,"Inside the if loop because lth is 0");
            switch (step) {
            case _STEP_FUNCTION:
                ...
                length_to_read = compute_meta_length_after_function(
                    ...
                    msg_type);
                if (length_to_read != 0) {
					ESP_LOGI(TAG,"Meta length after function is %d",length_to_read);
                    step = _STEP_META;
                    break;
            ...
            case _STEP_META:
                length_to_read = compute_data_length_after_meta(
                    ctx, msg, msg_type);
				ESP_LOGI(TAG,"Data length after meta is %d",length_to_read);
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    case MODBUS_FC_REPORT_SLAVE_ID: {
        ...
        str_len = strlen(LIBMODBUS_VERSION_STRING);
        memcpy(rsp + rsp_length, LIBMODBUS_VERSION_STRING, str_len);
        rsp_length += str_len;
    ...
    case MODBUS_FC_READ_EXCEPTION_STATUS:
        if (ctx->debug) {
            ESP_LOGE(TAG, "FIXME Not implemented\n");
        }
        errno = ENOPROTOOPT;
        return -1;
        break;
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    if (nb > MODBUS_MAX_WRITE_BITS) {
        if (ctx->debug) {
            ESP_LOGE(TAG, "ERROR Writing too many bits (%d > %d)\n",
                    nb, MODBUS_MAX_WRITE_BITS);
        }
        errno = EMBMDATA;
        return -1;
